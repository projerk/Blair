...
/**
 * A custom button for denying book return requests, implementing {@link Sender} and {@link MouseHandler}.
 * This button provides mouse interaction effects and sends a message when clicked.
 */
public class DenyReturnButton extends Button implements Sender, MouseHandler {

    /**
     * The socket client used to send messages to the server.
     */
    private static final SocketService client = SocketService.getInstance();

    /**
     * The deny icon image displayed on the button.
     */
    private static final Image image = FileHelper.getImage("close.png");

    /**
     * The ID of the book involved in the return request.
     */
    private int bookID;

    /**
     * The ID of the user who made the return request.
     */
    private int userID;

    /**
     * Default constructor for {@code DenyReturnButton}.
     */
    public DenyReturnButton() {
        // Default constructor
    }

    /**
     * Constructs a {@code DenyReturnButton} with the specified book ID and user ID.
     ...
     */
    public DenyReturnButton(int bookID, int userID) {
        setButtonStyle();
        handleMouseEvent();
        this.bookID = bookID;
        this.userID = userID;
    ...
    /**
     * Sends a message to the server to deny the return request for the specified book and user.
    ...
    public void sendMessage() {
        ...
        client.sendMessage("deny_return_book", data);
    }
